...
exp_node* copy_exp_tree(exp_node* root) {
    ...
    exp_node * node = new exp_node;
    node->type = root->type;
    node->val = root->val;
    node->child[0] = copy_exp_tree(root->child[0]);
    node->child[1] = copy_exp_tree(root->child[1]);
    //your code here
    return node;
    return NULL;
}
...
void analyze_if(see_state* state) {
    ...
    see_state * case1 = new see_state;
    see_state * case2 = new see_state;
    case1->cfg = state->cfg->next[0];
    case1->path_const = update_path_const(state,state->path_const,true,state->cfg->exp_tree);
    case1->sym_store = state->sym_store;
    case2->cfg = state->cfg->next[1];
    case2->path_const = update_path_const(state,state->path_const,false,state->cfg->exp_tree);
    case2->sym_store = state->sym_store;
    if(case1->cfg != NULL)
    state_queue.push(*case2);
    if(case2->cfg != NULL)
    state_queue.push(*case1);
    return;
}
